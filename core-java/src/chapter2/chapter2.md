...
ex) List<Integer> arr = new ArrayList<>();
...
ex) arr.set(0, 45); // 인덱스 0번 째 값을 45로 변경한다.
...
ArrayList<String> people = friends; // people과 friends는 같은 객체를 참조한다.
...
raiseSalary 메서드는 클래스의 인스턴스에 동작한다. 이런 메서드를 인스턴트 메서드라고 한다. 자바에서는 static을 선언하지 않은 메서드는 모두 인스턴스 메서드이다.
...
```
	...
	double prite() { return quantiry * unitPrice;}
   	}
