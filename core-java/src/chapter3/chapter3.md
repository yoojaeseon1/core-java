...
Serializable : 직렬화를 할지 말지

RandomAccess : randomAccess를 할지 말지

---

### Serializable(직렬화)

데이터를 파일에 쓰거나 네트워크로 전송하기 위해서 바이트 단위로 분해해서 순차적으로 보내는 것.

기본형 변수의 경우 정해진 바이트 변수이기 때문에 바이트 단위로 분해 후 전송 -> 전송 완료 후 다시 조립하는데 문제가 없다.

인스턴스의 경우 크기가 가변적이기 때문에 직렬화 하기 위해서는 Serializable 인터페이스를 구현해야 한다.


#### transient 제어자

- 직렬화 하고 싶지 않는 변수의 제어자로 사용한다.(파일 입출력, 네트워크 전송 등을 원치 않는 변수)
